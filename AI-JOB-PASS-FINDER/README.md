...
| 요구 | 파일 | 내용 |
|---|---|---|
| #1 정량 점수 공식 | `core/matching.py` | 내신 30 + 자격증 40 + 적합성 20 + 인재상 10 = 100점. 내신은 `30 × (5.0 − 등급) ÷ 4.0`, 자격증은 정확일치 100% / 유사자격 70% 인정비율의 평균 × 40점. **AI API 호출 없이 순수 파이썬 로컬 연산** (`core/spec_score.py`는 하위호환 재노출 레이어) |
| #2 예외처리·이중화 | `services/fallback.py` | 모든 외부 호출을 `safe_call()`로 감싸 예외를 흡수하고 `data/backup_master.json`(공고 34건·자격증 45종·강소기업 10건)으로 즉시 전환. `SourceTracker`가 소스별 출처·응답시간·실패사유를 기록해 상단 **[LIVE API] / [BACKUP DATA]** 컬러 배지로 표출 |
| #3 비용 차단막 | `services/llm.py` | `@st.cache_data(ttl=3600)` 로 동일 입력 재요청 시 API 재호출 차단. 키는 `st.secrets["CLAUDE_API_KEY"]`에서만 로드(하드코딩 없음), 캐시 키에도 포함하지 않음. 키 없으면 템플릿 생성기로 동작(호출 0회) |
| #4 OGQ 인터랙션 | `data/ogq_assets.py`, `assets/ogq/` | 점수 80↑ 축하 / 50~79 격려 / 50↓ 위로 스티커 자동 전환. 로드맵 3단계 체크 시 스탬프 이미지 렌더링. 로컬파일 → 웹 URL → 이모지 3단 폴백 |
| #5 The Cut | `app.py` 5번 탭 | 로그인·전문가 매칭 등을 MVP에서 제외한 이유와 '열림 조건'을 카드로 정리 |

### OGQ 이미지 교체 방법
`data/ogq_assets.py`의 `OGQ_WEB_URLS` 딕셔너리에 이미지 주소를 넣으면 웹 이미지가 우선 적용됩니다.
로컬 파일을 바꾸려면 `assets/ogq/` 안의 동일 파일명으로 덮어쓰면 됩니다.
