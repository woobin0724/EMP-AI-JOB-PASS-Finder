...
_JOSA_PAIRS = {
    ...
    "와": ("과", "와"),
    "로": ("으로", "로"),
    # 인용의 '이라/라' — 주격 '이/가'와 짝이 다르다.
    # ('태도'이라는 X / '태도'라는 O, '책임감'이라는 O)
    "라": ("이라", "라"),
}
_JOSA_RE = re.compile(r"#(를|은|이|와|로|라)")
...
_OPENERS = [
    ...
    "{dept}#은 제가 실습 시간마다 가장 오래 붙잡고 있던 일입니다.",
    "{company}#이 찾는 사람이 되기 위해 무엇을 준비했는지 적었습니다.",
...
_STRENGTH_LINES = [
    ...
    "무언가를 끝까지 맞춰놓는 편이라 '{strength}'#라는 평을 받아왔습니다.",
    "{strength} — 이것이 제가 현장에서 쓸 수 있는 가장 확실한 도구입니다.",
    "조원들은 저를 '{strength}'#로 기억합니다.",
